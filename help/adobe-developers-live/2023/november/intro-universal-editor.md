---
title: Introduktion till AEM Universal Editor
description: Universal Editor är nästa generation av AEM kontextredigerare och löser de nuvarande begränsningarna i AEM Page Editor eller SPA Editor. Det är oberoende av ramverk, återgivningsmotorer och datakällor, vilket ger all den frihet som utvecklarna vill ha och gör det helt universellt.
feature: Authoring
olution: Experience Manager,Experience Manager Sites
product: experience manager
sub-product: sites
topic: Development, Headless
role: Developer, User
level: Beginner, Intermediate
doc-type: Event
duration: 1848
last-substantial-update: 2023-12-20T00:00:00Z
jira: KT-14682
thumbnail: 3426341.jpeg
exl-id: 68d1e4bc-7f1a-40af-82bd-eb9b8f12ae44
source-git-commit: 460acb3fd1e9b29075cefa07e8d6947d2a61a314
workflow-type: tm+mt
source-wordcount: '114'
ht-degree: 0%

---

# Introduktion till AEM Universal Editor

Universal Editor är nästa generation av AEM kontextredigerare och löser de nuvarande begränsningarna i AEM Page Editor eller SPA Editor. Det är oberoende av ramverk, återgivningsmotorer och datakällor, vilket ger all den frihet som utvecklarna vill ha och gör det helt universellt.

>[!VIDEO](https://video.tv.adobe.com/v/3456471/?captions=swe&learn=on)
