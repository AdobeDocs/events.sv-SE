---
title: Börja med milstolpar
description: Upptäck enkla steg för att implementera milstolperapporter i Workfront för effektiv projektspårning och rapportering.
role: Admin, Leader, User
level: Beginner, Intermediate
doc-type: Event
duration: 208
last-substantial-update: 2025-12-17T00:00:00Z
jira: KT-18644
source-git-commit: 9f3e518295eaf64776880ba0442eb8a1b8d645e4
workflow-type: tm+mt
source-wordcount: '152'
ht-degree: 1%

---


# De tre viktigaste - Hur du kommer igång med milstolpar

Låt oss prata om milstolpe Paths! Det är din&quot;On-Demand Workfront CSM&quot; Cynthia Boon, och jag trodde att jag skulle dela min kärlek till Milestone Reports och hur enkelt det är att komma igång.

Det här är en bra rapport för ledningen på högre nivå och även för dina beställare som letar efter projektstatus, men som kanske inte behöver se alla detaljer om projekten.

Att ta med en milstolpe-rapport på din projektkontrollpanel eller dela den här rapporten (ännu bättre, automatisk schemaläggning för regelbunden leverans) är ett bra sätt att hålla berörda parter - även icke-Workfront-användare - engagerade i projektförloppet.

>[!VIDEO](https://video.tv.adobe.com/v/3470066/?learn=on&enablevpops)

## Ytterligare resurser

Mer information om milstolpar finns här:

* [Skapa en milstolpe-sökväg och milstolpe-steg](https://experienceleague.adobe.com/en/docs/workfront-learn/tutorials-workfront/administration-and-setup/approval-processes-and-milestone-paths/creating-milestones)
* [Använd milstolpe-vyn](https://experienceleague.adobe.com/en/docs/workfront/using/reporting/reports/report-elements/use-milestone-view)
* [Skicka och dela rapporter](https://experienceleague.adobe.com/en/docs/workfront-learn/tutorials-workfront/reporting/basic-reporting/how-to-send-and-share-reports)


