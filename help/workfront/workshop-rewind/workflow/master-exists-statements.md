---
title: Din dröm Workfront rapporterar EXISTS! Två metoder för att ändra EXISTS-satser
description: Tekniskt kunnande och praktiska applikationer som visar hur vi kan använda EXISTS-satser för att skapa smartare rapporter
role: Admin, Developer, Leader, User
level: Beginner, Intermediate, Experienced
doc-type: Event
duration: 3755
last-substantial-update: 2025-12-02T00:00:00Z
jira: KT-19787
source-git-commit: 2a5336b19f61bb0e1c48311b36c76c4a0967c410
workflow-type: tm+mt
source-wordcount: '165'
ht-degree: 0%

---


# Din dröm Workfront rapporterar EXISTS! Två metoder för att ändra EXISTS-satser

Vill du ta dina Workfront-rapporter till nästa nivå?  Se hur Skye Hansen och Nathan Johnson erbjöd två perspektiv för att skapa EXISTS-uttalanden som kan få dina rapporteringsdrömmar att förverkligas.

Under den här workshopen delade de tekniska kunskaper och praktiska program för att visa oss hur vi kan använda EXISTS-satser för att skapa smartare rapporter.

>[!VIDEO](https://video.tv.adobe.com/v/3477158/?learn=on&enablevpops)

Missade du livesessionen? Inga problem! Granska Skye &amp; Nathan&#39;s [presentationsdäck](https://experienceleaguecommunities.adobe.com/t5/workfront-discussions/event-follow-up-your-dream-workfront-report-exists-two/td-p/783550) och se hur on-demand-inspelningen följer med.

Har du idéer att dela eller har du frågor från evenemanget? Du kan släppa dem i kommentarerna på [Experience League Community-inlägget](https://workfront-experience.s3.us-west-2.amazonaws.com/Training/Guides/Customer+Success+at+Scale/10.28.25+EXISTS+Class.pdf)!

Nya händelser läggs till varje månad, så kontrollera [Experience League Events-sidan](https://experienceleague.adobe.com/en/events) för de senaste sessionerna.

